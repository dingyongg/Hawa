...
- (void)updateUserInfo:(WFCCUserInfo *)userInfo {
    ...
    NSData *extroData = [self.model.message.content.extra dataUsingEncoding:NSUTF8StringEncoding];
    NSDictionary *extro = [NSJSONSerialization JSONObjectWithData:extroData options:0 error:nil];

    if (extro) {

        [self.portraitView sd_setImageWithURL:[NSURL URLWithString:extro[@"avatar"]] placeholderImage:[UIImage imageNamed:@"PersonalChat"]];

    }else{
        [self.portraitView sd_setImageWithURL:[NSURL URLWithString:@""] placeholderImage:[UIImage imageNamed:@"PersonalChat"]];
    }
        
    if(self.model.showNameLabel) {
            self.nameLabel.text = extro[@"nickname"];
    }
...
- (void)setModel:(WFCUMessageModel *)model {
    ...
    if (model.message.direction == MessageDirection_Send) {
        ...
        if ([model.message.content isKindOfClass:[WFCCTextMessageContent class]]) {
            self.readStatusLabel.frame = CGRectMake(self.bubbleView.frame.origin.x-47, self.bubbleView.frame.origin.y+self.bubbleView.frame.size.height-15, 47,15);
            NSDate * now = [NSDate dateWithTimeIntervalSinceNow:0];
            NSTimeInterval inter = [now timeIntervalSince1970]*1000;
            NSLog(@"%f", inter - model.message.serverTime);
            if (inter - model.message.serverTime > 10*1000 ) {
                self.readStatusLabel.text = @"Readed";
            }else{
                self.readStatusLabel.text = @"Unread";
            }
        }
...
- (UILabel *)readStatusLabel {
    if (!_readStatusLabel) {
        _readStatusLabel = [[UILabel alloc] init];
        _readStatusLabel.frame = CGRectMake(0, 0, 50, 30);
        _readStatusLabel.numberOfLines = 1;
        _readStatusLabel.font = [UIFont systemFontOfSize:12];
        _readStatusLabel.userInteractionEnabled = YES;
        _readStatusLabel.text = @"Readed";
        _readStatusLabel.textColor = UIColor.lightGrayColor;
        [self.contentView addSubview:_readStatusLabel];
    }
    return _readStatusLabel;
}
