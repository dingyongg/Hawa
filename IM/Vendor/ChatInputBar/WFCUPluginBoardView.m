...
- (NSMutableArray *)pluginItems {
    if (!_pluginItems) {
        if (self.hasVoip) {
            _pluginItems = [@[
                              [[PluginItem alloc] initWithTitle:@"" image:[UIImage imageNamed:@"chat_input_plugin_album"] tag:1],
                              [[PluginItem alloc] initWithTitle:@"" image:[UIImage imageNamed:@"chat_input_plugin_video_call"] tag:2],
                              [[PluginItem alloc] initWithTitle:@"" image:[UIImage imageNamed:@"chat_input_plugin_audio_call"] tag:3],
#if WFCU_SUPPORT_VOIP
                              [[PluginItem alloc] initWithTitle:@"" image:[UIImage imageNamed:@"chat_input_plugin_gift"] tag:4]]
#endif
                               mutableCopy];
        } else {
            _pluginItems = [@[
